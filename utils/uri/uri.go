...
func (tu *helper.TsUri) ParseUriQueryToMap(query string) map[string]interface{} {
...
func (tu *helper.TsUri) GetQueryParams(mValues map[string][]string, key string) (value interface{}) {
...
// GetDomain 获取域名 .
func (tu *helper.TsUri) GetDomain(str string, isMains ...bool) (domain string) {
	u, err := url.Parse(str)
	isMain := false
	if len(isMains) > 0 {
		isMain = isMains[0]
...
func (tu *helper.TsUri) ParseUriQuery(m UriValues, query string) (map[string][]string, error) {
	var err error
	q := strings.Split(query, "?")
	if len(q) < 2 {
