...
func BenchmarkIsWindows(b *testing.B) {
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		helper.TOs.IsWindows()
	}
}

func TestIsLinux(t *testing.T) {
	res := helper.TOs.IsLinux()
	if !res {
...
func TestGetWorkList(t *testing.T) {
	res, err := helper.TOs.GetWorkList()
	if err != nil || res == "" {
...
func BenchmarkGetWorkList(b *testing.B) {
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = helper.TOs.GetWorkList()
	}
}

func TestChdir(t *testing.T) {
	err := helper.TOs.Chdir("./testdata")
	if err != nil {
		println(err.Error())
...
func BenchmarkChdir(b *testing.B) {
	...
	dir := helper.TOs.Pwd()
	for i := 0; i < b.N; i++ {
		_ = helper.TOs.Chdir(dir)
	}
}

func TestHomeDir(t *testing.T) {
	_, err := helper.TOs.HomeDir()
	if err != nil {
		t.Error("Pwd unit test fail")
		return
	}
...
func BenchmarkHomeDir(b *testing.B) {
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = helper.TOs.HomeDir()
	}
}

func TestLocalIP(t *testing.T) {
	_, err := helper.TOs.LocalIP()
	if err != nil {
...
func BenchmarkLocalIP(b *testing.B) {
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = helper.TOs.LocalIP()
	}
}

func TestOutboundIP(t *testing.T) {
	_, err := helper.TOs.OutboundIP()
	if err != nil {
...
func BenchmarkOutboundIP(b *testing.B) {
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = helper.TOs.OutboundIP()
	}
}

func TestIsPublicIP(t *testing.T) {
	ipStr, _ := helper.TOs.LocalIP()
	ipAddr := net.ParseIP(ipStr)
	if helper.TOs.IsPublicIP(ipAddr) {
		t.Error("IsPublicIP unit test fail")
		return
	}
	helper.TOs.IsPublicIP(net.ParseIP("127.0.0.1"))
	helper.TOs.IsPublicIP(net.ParseIP("172.16.0.1"))
	helper.TOs.IsPublicIP(net.ParseIP("192.168.0.1"))
	//google
	helper.TOs.IsPublicIP(net.ParseIP("172.217.26.142"))
	//google IPv6
	helper.TOs.IsPublicIP(net.ParseIP("2404:6800:4005:80f::200e"))
}
...
func BenchmarkIsPublicIP(b *testing.B) {
	...
	ipAddr := net.ParseIP(ipStr)
	for i := 0; i < b.N; i++ {
		helper.TOs.IsPublicIP(ipAddr)
	}
}

func TestGetIPs(t *testing.T) {
	ips := helper.TOs.GetIPs()
	if len(ips) == 0 {
...
func BenchmarkGetMacAddrs(b *testing.B) {
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		helper.TOs.GetMacAddrArr()
	}
}

func TestHostname(t *testing.T) {
	res, err := helper.TOs.Hostname()
	if err != nil || res == "" {
...
func BenchmarkHostname(b *testing.B) {
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = helper.TOs.Hostname()
	}
}

func TestGetIpByHostname(t *testing.T) {
	name := "localhost"
	ip, err := helper.TOs.GetIpByHostname(name)
	if err != nil || ip != "127.0.0.1" {
		t.Error("GetIpByHostname unit test fail")
		return
	}

	_, _ = helper.TOs.GetIpByHostname("::1")
	_, _ = helper.TOs.GetIpByHostname("hello")
}
...
func BenchmarkGetIpByHostname(b *testing.B) {
	...
	name := "localhost"
	for i := 0; i < b.N; i++ {
		_, _ = helper.TOs.GetIpByHostname(name)
	}
}

func TestGetIpsByDomain(t *testing.T) {
	name := "google.com"
	ips, err := helper.TOs.GetIpsByDomain(name)
	if err != nil || len(ips) == 0 {
		t.Error("GetIpsByDomain unit test fail")
		return
	}

	ips, err = helper.TOs.GetIpsByDomain("hello")
	if err == nil || len(ips) > 0 {
		t.Error("GetIpsByDomain unit test fail")
		return
	}
...
func BenchmarkGetIpsByDomain(b *testing.B) {
	...
	name := "google.com"
	for i := 0; i < b.N; i++ {
		_, _ = helper.TOs.GetIpsByDomain(name)
	}
}

func TestGetHostByIp(t *testing.T) {
	ip := "127.0.0.1"
	host, err := helper.TOs.GetHostByIp(ip)
	if err != nil || host == "" {
		t.Error("GetHostByIp unit test fail")
		return
	}

	_, _ = helper.TOs.GetHostByIp("192.168.1.1")
}
...
func BenchmarkGetHostByIp(b *testing.B) {
	...
	ip := "127.0.0.1"
	for i := 0; i < b.N; i++ {
		_, _ = helper.TOs.GetHostByIp(ip)
	}
}

func TestGoMemory(t *testing.T) {
	mem := helper.TOs.GoMemory()
	if mem == 0 {
...
func TestCpuUsage(t *testing.T) {
	usr, idle, total := helper.TOs.CpuUsage()
...
func TestSetenvGetenv(t *testing.T) {
	...
	err := helper.TOs.Setenv(name1, "world")
	if err != nil {
		t.Error("Setenv unit test fail")
		return
	}

	val1 := helper.TOs.Getenv(name1)
	val2 := helper.TOs.Getenv(name2)
	if val1 != "world" || val2 == "" {
...
func BenchmarkSetenv(b *testing.B) {
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = helper.TOs.Setenv("HELLO", "world")
	}
}
...
func TestGetEndian_IsLittleEndian(t *testing.T) {
	...
	endi := helper.TOs.GetEndian()
	isLit := helper.TOs.IsLittleEndian()
...
func BenchmarkIsLittleEndian(b *testing.B) {
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		helper.TOs.IsLittleEndian()
	}
}

func TestExec(t *testing.T) {
	cmd := " ls -a -h"
	ret, _, _ := helper.TOs.Exec(cmd)
	if ret == 1 {
...
func TestSystem(t *testing.T) {
	...
	_, _, _ = helper.TOs.System(cmd)
	_ = helper.TFile.DelDir("./testdata/empty", false)
}
...
func TestChmodChown(t *testing.T) {
	file := "./testdata"
	res1 := helper.TOs.Chmod(file, 0777)

	usr, _ := user.Current()
	uid := helper.TConv.Str2Int(usr.Uid)
	guid := helper.TConv.Str2Int(usr.Gid)

	res2 := helper.TOs.Chown(file, uid, guid)
...
func BenchmarkChown(b *testing.B) {
	...
	guid := helper.TConv.Str2Int(usr.Gid)
	for i := 0; i < b.N; i++ {
		helper.TOs.Chown(file, uid, guid)
	}
}

func TestGetTempDir(t *testing.T) {
	res := helper.TOs.GetTempDir()
	if res == "" {
...
func BenchmarkGetTempDir(b *testing.B) {
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		helper.TOs.GetTempDir()
	}
}

func TestPrivateCIDR(t *testing.T) {
	res := helper.TOs.PrivateCIDR()
	if len(res) == 0 {
...
func BenchmarkPrivateCIDR(b *testing.B) {
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		helper.TOs.PrivateCIDR()
	}
}

func TestIsPrivateIp(t *testing.T) {
	// 无效Ip
	res, err := helper.TOs.IsPrivateIp("hello")
	if res || err == nil {
	...
	// TPrivyCiders未初始化数据
	if len(helper.TPrivyCiders) != 0 {
		t.Error("IsPrivateIp unit test fail")
		return
	}

	// docker ip
	res, err = helper.TOs.IsPrivateIp("172.17.0.1")
	if !res || err != nil {
		t.Error("IsPrivateIp unit test fail")
		return
	}
	...
	//外网ip
	res, err = helper.TOs.IsPrivateIp("220.181.38.148")
	if res || err != nil {
		t.Error("IsPrivateIp unit test fail")
		return
	}
...
func BenchmarkIsPrivateIp(b *testing.B) {
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = helper.TOs.IsPrivateIp("172.17.0.1")
	}
}
...
func TestClientIp(t *testing.T) {
	...
	for _, v := range testData {
		actual = helper.TOs.ClientIp(v.request)
		if v.expected == "::1" {
			if actual != "127.0.0.1" {
...
func BenchmarkClientIp(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		helper.TOs.ClientIp(req)
	}
}

func TestGetSystemInfo(t *testing.T) {
	info := helper.TOs.GetSystemInfo()
	fmt.Printf("%+v\n", info)
}
...
func TestIsPortOpen(t *testing.T) {
	...
	for _, test := range tests {
		actual := helper.TOs.IsPortOpen(test.host, test.port, test.protocol)
		if actual != test.expected {
			t.Errorf("Expected IsChineseName(%q, %v, %q) to be %v, got %v", test.host, test.port, test.protocol, test.expected, actual)
		}
	}

	helper.TOs.IsPortOpen("127.0.0.1", 80, "tcp")
	helper.TOs.IsPortOpen("::", 80, "tcp")
	helper.TOs.IsPortOpen("::", 80, "")
	helper.TOs.IsPortOpen("::", 80)
}
...
func TestGetPidByPortGetProcessExeByPid(t *testing.T) {
	...
	time.AfterFunc(time.Millisecond*200, func() {
		helper.getPidByInode("1234", nil)
		helper.TOs.GetPidByPort(22)
		helper.TOs.GetPidByPort(25)
		helper.TOs.GetPidByPort(1999)
		res := helper.TOs.GetPidByPort(2020)
		exePath := helper.TOs.GetProcessExeByPid(res)
		if res == 0 {
...
func BenchmarkGetProcessExeByPid(b *testing.B) {
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		helper.TOs.GetProcessExeByPid(2020)
	}
}
