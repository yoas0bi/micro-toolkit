...
// Echo 输出一个或多个字符串.
func (to *helper.TsOs) Echo(args ...interface{}) {
	log.Print(args...)
}

// Exit 输出一条消息,并退出当前脚本.
func (to *helper.TsOs) Exit(msg interface{}, status int) {
	log.Println(msg)
	os.Exit(status)
}

// Die 函数输出一条消息,并退出当前脚本.
func (to *helper.TsOs) Die(msg interface{}, status int) {
	log.Println(msg)
	os.Exit(status)
}

// IsWindows 当前操作系统是否Windows.
func (to *helper.TsOs) IsWindows() bool {
	return "windows" == runtime.GOOS
}

// IsLinux 当前操作系统是否Linux.
func (to *helper.TsOs) IsLinux() bool {
	return "linux" == runtime.GOOS
}

// IsMac 当前操作系统是否Mac OS/X.
func (to *helper.TsOs) IsMac() bool {
	return "darwin" == runtime.GOOS
}

// Pwd 获取当前程序运行所在的路径,注意和GetWorkList有所不同.
func (to *helper.TsOs) Pwd() (dir string) {
	var ex string
	var err error
...
func (to *helper.TsOs) GetWorkList() (dir string, err error) {
	dir, err = os.Getwd()
	return
}

// Chdir 改变/进入新的工作目录.
func (to *helper.TsOs) Chdir(dir string) error {
	return os.Chdir(dir)
}

// HomeDir 获取当前用户的主目录(仅支持Unix-like system).
func (to *helper.TsOs) HomeDir() (string, error) {
...
func (to *helper.TsOs) LocalIP() (string, error) {
	ip := ""
...
func (to *helper.TsOs) OutboundIP() (string, error) {
	out := ""
...
// IsPublicIP 是否公网IP.
func (to *helper.TsOs) IsPublicIP(ip net.IP) bool {
...
// GetIPs 获取本机的IP列表.
func (to *helper.TsOs) GetIPs() (ips []string) {
...
func (to *helper.TsOs) GetMacAddrArr() (macAddrArr []string) {
...
// Hostname 获取主机名.
func (to *helper.TsOs) Hostname() (string, error) {
	return os.Hostname()
}

// GetIpByHostname 返回主机名对应的 IPv4地址.
func (to *helper.TsOs) GetIpByHostname(hostname string) (string, error) {
...
func (to *helper.TsOs) GetIpsByDomain(domain string) ([]string, error) {
...
func (to *helper.TsOs) GetHostByIp(ipAddress string) (string, error) {
...
func (to *helper.TsOs) GoMemory() uint64 {
...
// idle为空闲时间,
// total为累计时间.
func (to *helper.TsOs) CpuUsage() (user, idle, total uint64) {
...
// Setenv 设置一个环境变量的值.
func (to *helper.TsOs) Setenv(varName, data string) error {
	return os.Setenv(varName, data)
}

// Getenv 获取一个环境变量的值.
func (to *helper.TsOs) Getenv(varName string) string {
	return os.Getenv(varName)
}

// GetEndian 获取系统字节序类型,小端返回binary.LittleEndian,大端返回binary.BigEndian .
func (to *helper.TsOs) GetEndian() binary.ByteOrder {
	return helper.getEndian()
}

// IsLittleEndian 系统字节序类型是否小端存储.
func (to *helper.TsOs) IsLittleEndian() bool {
...
// "ls -a"
// "/bin/bash -c \"ls -a\""
func (to *helper.TsOs) Exec(command string) (retInt int, outStr, errStr []byte) {
	// split command
	q := rune(0)
...
func (to *helper.TsOs) System(command string) (retInt int, outStr, errStr []byte) {
	// split command
	q := rune(0)
	parts := strings.FieldsFunc(command, func(r rune) bool {
		switch {
...
// Chmod 改变文件模式.
func (to *helper.TsOs) Chmod(filename string, mode os.FileMode) bool {
	return os.Chmod(filename, mode) == nil
}

// Chown 改变文件的所有者.
func (to *helper.TsOs) Chown(filename string, uid, gid int) bool {
	return os.Chown(filename, uid, gid) == nil
}

// GetTempDir 返回用于临时文件的目录.
func (to *helper.TsOs) GetTempDir() string {
	return os.TempDir()
}

// PrivateCIDR 获取私有网段的CIDR(无类别域间路由).
func (to *helper.TsOs) PrivateCIDR() []*net.IPNet {
...
func (to *helper.TsOs) IsPrivateIp(address string) (bool, error) {
	ip := net.ParseIP(address)
	if ip == nil {
		return false, errors.New("address is not valid ip")
	}

	if helper.TPrivyCiders == nil {
		helper.TPrivyCiders = to.PrivateCIDR()
	}
	for i := range helper.TPrivyCiders {
		if helper.TPrivyCiders[i].Contains(ip) {
			return true, nil
		}
	}
...
func (to *helper.TsOs) ClientIp(req *http.Request) string {
	// 获取头部信息,有可能是代理
...
func (to *helper.TsOs) GetSystemInfo() *SystemInfo {
	//运行时信息
	...
	return &SystemInfo{
		...
		Runtime:      int64(helper.TTime.ServiceUptime()),
...
func (to *helper.TsOs) IsPortOpen(host string, port interface{}, protocols ...string) bool {
	if helper.TValidate.IsHost(host) && helper.TValidate.IsPort(port) {
		// 默认tcp协议
		protocol := "tcp"
		...
		conn, _ := net.DialTimeout(protocol, net.JoinHostPort(host, helper.TConv.ToStr(port)), helper.CheckConnectTimeout)
		if conn != nil {
			_ = conn.Close()
			return true
		}
...
func (to *helper.TsOs) GetPidByPort(port int) (pid int) {
	files := []string{
		"/proc/net/tcp",
		"/proc/net/udp",
		"/proc/net/tcp6",
	...
	for _, filePath := range files {
		lines, _ := helper.TFile.ReadInArray(filePath)
		for _, line := range lines[1:] {
			...
			locPort, _ := helper.TConv.Hex2Dec(ipport[1])

			// 非该端口
			if int(locPort) != port {
				continue
			}
			pid = helper.getPidByInode(fields[9], procDirs)
			if pid > 0 {
				return
			}
		}
...
func (to *helper.TsOs) GetProcessExeByPid(pid int) string {
	return helper.getProcessExeByPid(pid)
}

// ForceGC 强制手动GC垃圾回收(阻塞).
func (to *helper.TsOs) ForceGC() {
	runtime.GC()
	debug.FreeOSMemory()
}

// TriggerGC 触发GC(非阻塞).
func (to *helper.TsOs) TriggerGC() {
	go func() {
		to.ForceGC()
	}()
}
